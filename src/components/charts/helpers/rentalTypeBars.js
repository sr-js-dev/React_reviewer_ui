 ...
 * sent by Roman
 *
 * ALSO: `null` value indicates here that value is hidden due to
 * plan restriction. In this case, we should replace that value
 * with placeholder AND make style different (grey-out bars)
 *
 * Essentially
 */
export default function(restrictedPattern = null, gradients = []) {
  return function() {
    ...
    if (restricted) {
      ...
      if (this._datasetIndex === 1) {
        ctx.fillStyle = ctx.createPattern(restrictedPattern, 'repeat');
      } else {
        ctx.fillStyle = '#fff';
      }
    } else {
      ...
      //  A HACK! :D
      //
      //  I pass two linear gradient colors in labels so I can access them here
      //      If colors are passed in label, they are passed for a dataset
      //      so we should parse that;
      if (this._model.datasetLabel.indexOf(', #') !== -1) {
        linearGradientColors = this._model.datasetLabel.split(', ');
        //      If not, they should be passed as an argument for and index
        //      in dataset
      } else {
        linearGradientColors = gradients[this._index];
      }
      ...
      fillGradient.addColorStop(0, linearGradientColors[0]);
      fillGradient.addColorStop(1, linearGradientColors[1]);
